...
def dSdt_array(SIRxt = [], *args):
    ...
    dS_dt_array[:] = - infecRate*S[:]*crossI_neighborSum_X(I, alva.eventName, gX)[:]                      + inOutRate*totalSIR - inOutRate*S[:]
    return(dS_dt_array)
...
def dRdt_array(SIRxt = [], *args):
    ...
    # each dIdt with the same equation form
    dR_dt_array[:] = + recovRate*I[:] - inOutRate*R[:]                      - infecRate*S[:]*I[:]                      + infecRate*S[:]*crossI_neighborSum_X(I, alva.eventName, gX)[:]
    return(dR_dt_array)
