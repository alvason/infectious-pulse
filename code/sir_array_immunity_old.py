...
plt.text(0, 4.0/6,r'$ \frac{\partial S_n(t)}{\partial t} =          -\beta S_n(t)\sum_{\eta = n_{min}}^{n_{max}} (1 - \frac{|n - \eta|}{r + |n - \eta|})I_{\eta}(t) + \mu N - \mu S_n(t)$'
         , fontsize = 1.2*AlvaFontSize)
plt.text(0, 2.0/6, r'$ \frac{\partial I_n(t)}{\partial t} =          +\beta S_n(t)I_n(t) - \gamma I_n(t) - \mu I_n(t)          + m \frac{I_{n - 1}(t) - 2I_n(t) + I_{n + 1}(t)}{(\Delta n)^2} $'
         , fontsize = 1.2*AlvaFontSize)
plt.text(0, 0.0/6,r'$ \frac{\partial R_n(t)}{\partial t} =          +\gamma I_n(t) - \mu R_n(t) - \beta S_n(t)I_n(t)         + \beta S_n(t)\sum_{\eta = n_{min}}^{n_{max}} (1 - \frac{|n - \eta|}{r + |n - \eta|})I_{\eta}(t) $'
         , fontsize = 1.2*AlvaFontSize)
plt.show()
...
def dIdt_array(SIRxt = [], *args):
    ...
    rightX[-1] = centerX[-1]
    for xn in range(x_totalPoint):
        dI_dt_array[xn] = + infecRate*S[xn]*I[xn]                           - recovRate*I[xn] - inOutRate*I[xn]                           + mutatRate*(leftX[xn] - 2*centerX[xn] + rightX[xn])/(dx**2)                                                                                             
    return(dI_dt_array)
...
def dRdt_array(SIRxt = [], *args):
    ...
    # each dIdt with the same equation form
    for xn in range(x_totalPoint):
        dR_dt_array[xn] = + recovRate*I[xn] - inOutRate*R[xn]                           - infecRate*S[xn]*I[xn]                           + infecRate*S[xn]*crossInfect(cross_radius, x_totalPoint, I, xn)
    return(dR_dt_array)
